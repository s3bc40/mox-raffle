...
@external
def fundSubscription(sub_id: uint256, amount: uint256):
    assert self.subscription_config[sub_id].owner != empty(address), INVALID_SUBSCRIPTION
    
    old_balance: uint256 = self.subscriptions[sub_id].balance
...
@internal
def fulfillRandomWordsWithOverride(request_id: uint256, consumer: address, words: DynArray[uint256, MAX_WORD_SIZE]):
    """Returns an array of numbers to the consumer contract (from raw_call)
    ...
    callReq: Bytes[3236] = abi_encode(request_id, words, method_id=method_id("rawFulfillRandomWords(uint256,uint256[])"))
    
    self.config.reentrancy_lock = True
    success: bool = False
    response: Bytes[32] = b""
    success, response = raw_call(consumer, callReq, max_outsize=32, revert_on_failure=False)
    self.config.reentrancy_lock = False

    log RandomWordsFulfilled(request_id, request_id, request_id, 0, True, success, False)
